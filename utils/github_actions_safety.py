"""Transitional GitHub Actions safety helpers.

When ``GITHUB_ACTIONS=true``, collection must be local-only and the legacy
Daily Pipeline must not LLM-generate or publish. These helpers are the
runtime kill switch while the remote YAML cannot be replaced (PAT lacks
``workflow`` scope).

TODO: After a PAT with ``workflow`` scope pushes the slim Daily Collection
workflow, keep or delete these guards once remote YAML no longer generates,
publishes, or injects secrets. Do not weaken the unit tests until then.
"""
