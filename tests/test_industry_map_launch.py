...
def test_article_explains_revision_and_publishes_sourced_v02_samples():
    ...
    assert "合豚科技" in article["content"]
    assert "嗨便利" in article["content"]
    assert "映翰通" in article["content"]
    assert "苏州交投能源" in article["content"]
    assert "上海商学院" in article["content"]
    assert "李政道研究所" in article["content"]
    assert "雷允上" in article["content"]
    assert "样本不等于全量名录" in article["content"]
...
def test_update_verified_draft_reuses_existing_media_id_and_reads_back():
    client = FakeDraftClient()
    article = build_industry_map_article(
        ["https://mmbiz.qpic.cn/one", "https://mmbiz.qpic.cn/two"],
        thumb_media_id="cover-media-id",
    ...
    media_id = update_verified_draft(client, "draft-media-id", article)
